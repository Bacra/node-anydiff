...
Closest.prototype = {
	...
	eqEqualMap: function(index) {
		...
		if (!xy) return [this._genEqualMap(this.start, this.end, 4)];
	 ...
	 * @param  {Object} start
	 * @param  {Object} end
	 * @return {Number} 0: 无效或相等的区间 1: 新删除  2: 新增加 3: 相等 4:  替换  -1: 依旧混乱
	 */
	_intervalStat: function(start, end) {
		...
		if (start.x == end.x -1) return 1;
		if (start.y == end.y -1) return 2;
